...
`go-webmvc` 是一个用于学习与快速启动的 Go Web 服务模板，采用典型的企业应用分层：handler → service → repository → model.

`解决什么问题?`  帮助开发者快速搭建一个结构清晰、易于维护的 Go Web 项目骨架， 避免每次新建项目都要从零开始配置各种常用组件。 省时省力,统一项目结构, 提升开发效率, 对新手入门go开发友好。
