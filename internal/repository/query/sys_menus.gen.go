...
func newSysMenu(db *gorm.DB, opts ...gen.DOOption) sysMenu {
	...
	_sysMenu.Router = field.NewString(tableName, "router")
	_sysMenu.MenuType = field.NewInt(tableName, "menu_type")
	_sysMenu.ReqAction = field.NewString(tableName, "req_action")
	...
	_sysMenu.SysRole = sysMenuManyToManySysRole{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("SysRole", "model.SysRole"),
		SysMenu: struct {
			field.RelationField
			SysRole struct {
				field.RelationField
			...
			RelationField: field.NewRelation("SysRole.SysMenu", "model.SysMenu"),
			SysRole: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("SysRole.SysMenu.SysRole", "model.SysRole"),
			},
		},
	}
...
type sysMenu struct {
	...
	Status     field.Int
	SysRole    sysMenuManyToManySysRole
...
func (s *sysMenu) updateTableName(table string) *sysMenu {
	...
	s.Router = field.NewString(table, "router")
	s.MenuType = field.NewInt(table, "menu_type")
	s.ReqAction = field.NewString(table, "req_action")
...
func (s *sysMenu) fillFieldMap() {
	...
	s.fieldMap["router"] = s.Router
	s.fieldMap["menu_type"] = s.MenuType
	s.fieldMap["req_action"] = s.ReqAction
...
type sysMenuManyToManySysRole struct {
	db *gorm.DB
...
func (a sysMenuManyToManySysRole) Where(conds ...field.Expr) *sysMenuManyToManySysRole {
	if len(conds) == 0 {
		return &a
	}
...
func (a sysMenuManyToManySysRole) WithContext(ctx context.Context) *sysMenuManyToManySysRole {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a sysMenuManyToManySysRole) Session(session *gorm.Session) *sysMenuManyToManySysRole {
	a.db = a.db.Session(session)
	return &a
}

func (a sysMenuManyToManySysRole) Model(m *model.SysMenu) *sysMenuManyToManySysRoleTx {
	return &sysMenuManyToManySysRoleTx{a.db.Model(m).Association(a.Name())}
}

func (a sysMenuManyToManySysRole) Unscoped() *sysMenuManyToManySysRole {
	a.db = a.db.Unscoped()
	return &a
}

type sysMenuManyToManySysRoleTx struct{ tx *gorm.Association }

func (a sysMenuManyToManySysRoleTx) Find() (result []*model.SysRole, err error) {
	return result, a.tx.Find(&result)
}

func (a sysMenuManyToManySysRoleTx) Append(values ...*model.SysRole) (err error) {
...
func (a sysMenuManyToManySysRoleTx) Replace(values ...*model.SysRole) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a sysMenuManyToManySysRoleTx) Delete(values ...*model.SysRole) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a sysMenuManyToManySysRoleTx) Clear() error {
	return a.tx.Clear()
}

func (a sysMenuManyToManySysRoleTx) Count() int64 {
	return a.tx.Count()
}

func (a sysMenuManyToManySysRoleTx) Unscoped() *sysMenuManyToManySysRoleTx {
	a.tx = a.tx.Unscoped()
	return &a
}
...
type ISysMenuDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []model.SysMenu, err error)
}

// SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (s sysMenuDo) FilterWithNameAndRole(name string, role string) (result []model.SysMenu, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, name)
	generateSQL.WriteString("SELECT * FROM sys_menus WHERE name = ? ")
	if role != "" {
